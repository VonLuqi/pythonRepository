...
def main():
    ...
    for i in range(3):
        ...
        for i in range(2):
            while True:
                numero_atual = input("Digite seu número de telefone: ((XX) 9XXXX-XXXX)\n\n")
                if validar_telefone(numero_atual):
                    telefone_usuario.append(numero_atual)
                    limpar_tela()
                    break
        
        telefone_formatados_str = ", ".join(telefone_usuario)
        print(f"Bem-vindo {nome_usuario}! Telefones {telefone_formatados_str} cadastrados com sucesso!\n")
        t.sleep(3)
        limpar_tela()
